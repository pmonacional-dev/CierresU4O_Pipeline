...
def ProcesoCentralizado(seleccion, origen):
    # Proceso para QLIK (Restaurado y ajustado para Acompañamiento)
    if seleccion == "QLIK":
        try:
            ...
            # Envios a datamart de procesos de extracción para indicadores
            # Origen: ETL Local -> Destino: QLIK SaaS
            ...
            # Solo "Extraer_Acompañamiento_Porcentaje" en la lista de tablas
            tables_to_process = ["Extraer_Acompañamiento_Porcentaje"]
            ...
            try:
                for table_name in tables_to_process:
                    ...
                    logging.info(f"Data from selected {modified_table_name} deleted successfully from QLIK.")
                
                central_connSqlServer.commit()
            
            except Exception as delete_error:
                logging.error(f"Error in record deletion (QLIK): {delete_error}")
            ...
            try:
                for table_name in tables_to_process:
                    ...
                    while True:
                        ...
                        logging.info(
                            f"Datamart Qlik Azure | {modified_table_name} : Batch of {len(batch)} rows inserted successfully.")
            
            except pyodbc.Error as e:
                logging.error(f"SQL Server Error (QLIK): {e}")
            finally:
                if local_sql_cursor:
            ...
            logging.error(f"Error in main function (QLIK): {main_error}")
    
    # Proceso para SINDATA
        ...
        try:
            ...
            # Origen: ETL Local -> Destino: SINDATA SaaS
            local_connSqlServer, local_sql_cursor = Conexiones.connect_ETL_local_sql(origen)
            ...
            try:
                for table_name in tables_to_process:
                    ...
                    delete_query = f"DELETE FROM {modified_table_name}"
                    central_sql_cursor.execute(delete_query)
                    logging.info(f"Data from selected {modified_table_name} deleted successfully from SINDATA.")
                
                central_connSqlServer.commit()
            
            except Exception as delete_error:
                logging.error(f"Error in record deletion (SINDATA): {delete_error}")
            ...
            try:
                ...
                logging.error(f"SQL Server Error (SINDATA): {e}")
            finally:
                if local_sql_cursor:
                    local_sql_cursor.close()
                    local_connSqlServer.close()
            ...
            logging.error(f"Error in main function (SINDATA): {main_error}")


def main(origen, recurrencia=None):
    startTime = datetime.now()
    logging.info('U4O_Centralizar_Acompañamiento | Inicia ejecución ')
    
    # Se ejecutan ambos procesos: QLIK y SINDATA
    ProcesoCentralizado("QLIK", origen)
    ProcesoCentralizado("SINDATA", origen)
    
    logging.info(f'U4O_Centralizar_Acompañamiento | Tiempo de ejecución: {datetime.now() - startTime}')
